...
$url_prefix = $CORE->configuration["site"] . "?section=task&task=";
...
if (isset($_REQUEST["oper"]) && in_array($_REQUEST["oper"], array('json', 'update', 'search', 'tasks'))) {
    ...
    switch ($_REQUEST["oper"]) {
        ...
        case 'tasks':
            ...
            break;
        case 'json':
            $data = prepeareData4Graph($CORE, $conn, $taskid);
            $newdata = array();
            for ($i = 0; $i < count($data); $i++) {
...
if (isset($_REQUEST["task"])) {
    ...
    if ((isset($_SESSION['project'])) && (($_SESSION["project"]) != "all")) {
