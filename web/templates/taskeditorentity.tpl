                ...
                <tr>
                    ...
                    <td style="text-align:left">
                        <table class="container" style="width:10%; border:0px;">
                            <tr>
                                <td rowspan="2" style="border:0px;"><img src="{$factory->img_admin_dir}priority_{$task.fm_priority_name}.gif" alt="{$task.fm_priority_name}"
                                                                         title="{$task.fm_priority_descr}"/></td>
                                <td style="border:0px; valign:bottom;">{if $task.fm_priority ne 1}<a href="#" onclick="changeFieldImg('fm_priority', '{$task.fm_priority-1}');"><img
                                                src="{$factory->img_admin_dir}rateup.png" alt="priority_up" title="Поднять приоритет"/></a>{/if}</td>
                            </tr>
                            <tr>
                                <td style="border:0px; ">{if $task.fm_priority ne 3}<a href="#" onclick="changeFieldImg('fm_priority', '{$task.fm_priority+1}');"><img
                                                src="{$factory->img_admin_dir}ratedown.png" alt="priority_down" title="Понизить приоритет"/></a>{/if}</td>
                            </tr>
                        </table>
                    </td>
                </tr>
            ...
            <table class="container">
                ...
                {section name=i loop=$task.child}
                    <tr>
                        ...
                        <td>
                            <img src="{$factory->img_admin_dir}priority_{$task.child[i].fm_priority_name}.gif" alt="{$task.child[i].fm_priority_name}"
                                 title="{$task.fm_priority_descr}"/>
                        </td>
                        <td>
                ...
                <table>
                    ...
                    <table class="container">
                        ...
                        {section name=i loop=$task.parent}
                            <tr>
                                <td style="text-align:left; width:90%">
                                    <a href="?task={$task.parent[i].id}" style="font-size:8pt">{$task.parent[i].fm_name}&nbsp;{$task.parent[i].fm_code}
                                        .{$task.parent[i].fm_descr}</a>
                                </td>
                                <td>
                                ...
                                <td>
                                    <img src="{$factory->img_admin_dir}priority_{$task.parent[i].fm_priority_name}.gif" alt="{$task.parent[i].fm_priority_name}"
                                         title="{$task.parent[i].fm_priority_descr}"/>
                                </td>
                                <td>
                                    <img src="{$factory->img_admin_dir}state_{$task.parent[i].fm_state_name}.gif" alt="{$task.parent[i].fm_state_name}"
                                         title="{$task.parent[i].fm_state}"/>
                                </td>
                                <td>
...
<div style="">
    ...
    <div>
        ...
        <form action="">
            ...
            <table class="container" id="worklog" style="font-size:9pt;">
                ...
                {section name=i loop=$task.worklog}
                    <tr>
                        <td>{$task.worklog[i].fm_date|date_format:$const.dateformat}</td>
                        <td>
                            <img src="{$factory->img_admin_dir}log_cat_{$task.worklog[i].fm_cat_name}.gif" alt="{$task.worklog[i].fm_cat_name}"
                                 title="{$task.worklog[i].fm_cat_descr}"/>
                        </td>
                ...
                <tr id="workloginput">
                    <td>
                        <input type="text" size="10" name="fm_date" value="{$smarty.now|date_format:$const.dateformat}" class="corp_text"/>
                    </td>
                    <td style="text-align:left;">
                        <select name="fm_cat">
                            {section name=i loop=$task.worklog_cat}
                                <option value="{$task.worklog_cat[i].id}"
                                        {if $task.worklog_cat[i].fm_name eq 'cmnt'}selected="selected"{/if}>{$task.worklog_cat[i].fm_descr}</option>
                            {/section}
                        </select>
                    </td>
                    <td style="text-align:left;">
        ...
        <div style="display:none;" id="inputPopupPromt" title="Изменить значение поля">
            ...
            <p>
            <div id="inputFieldError" class="ui-state-error ui-corner-all" style="display:none;"></div>
            </p>
        </div>
        <div style="display:none;" id="selectPopupPromt" title="Изменить проект">
            ...
            <p>
            <div id="selectFieldError" class="ui-state-error ui-corner-all" style="display:none;"></div>
            </p>
        </div>
