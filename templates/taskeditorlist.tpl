...
<table class="container">
   ...
   <tr>
	 <td>
	    <a href="?task={$tasks[i].id}">{$tasks[i].tm_project}-{$tasks[i].id}</a>
	</td>
	<td style="text-align:left;">
	   <a href="?task={$tasks[i].id}">
		{$tasks[i].tm_code}.{$tasks[i].tm_descr}
	   </a>
	</td>
	<td> <a href="?dep=user&user={$tasks[i].tm_user}">{$tasks[i].tm_user}</a></td>
	<td>
	  {if $tasks[i].tm_state_name eq 'done'}
           	100
          {else}
   ...
   <tr>
	<td>
	   <select name="tm_project" style="corp">
		{section name=i loop=$projects}
		<option value="{$projects[i].id}" {if $projects[i].id eq $smarty.request.project}selected="selected"{/if}>{$projects[i].tm_name}</option>
		{/section}
	   </select>
	</td>
	<td style="text-align:left;">
	   <input type="text" name="tm_name" size="20" class="corp_text"/>
	   <input type="text" name="tm_descr" size="70" class="corp_text"/>
	</td>
	<td style="text-align:center;">
	   {$smarty.session.user.samaccountname}
	</td>
	<td style="text-align:center;">
	   <input type="text" name="tm_plan" size="10" class="corp_text"/>
	</td>
	<td>
